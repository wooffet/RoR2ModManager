import { ipcMain, IpcMainEvent, BrowserWindow, protocol } from 'electron';
...
export const registerIpcListeners = () => {
  ipcMain.on('openDialog', (event: IpcMainEvent, opts: DialogWindowOptions) => {
    if (!opts.slug)
  ...
  ipcMain.on('registerHttp', (event: IpcMainEvent, scheme: string) => {
